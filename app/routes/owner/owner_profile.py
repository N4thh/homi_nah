"""
Owner Profile Routes - Profile management and settings functionality
"""
...
owner_profile_bp = Blueprint('owner_profile', __name__, url_prefix='/owner')


class OwnerProfileHandler(BaseRouteHandler):
    """Handler for owner profile functionality"""
    
    def __init__(self):
        super().__init__('owner_profile')
...
@owner_profile_bp.route('/profile', methods=['GET', 'POST'])
...
def profile():
    ...
    return render_template('owner/profile.html', owner=current_user)


@owner_profile_bp.route('/update_profile', methods=['POST'])
@login_required
@handle_validation_errors
def update_profile():
    """Update owner profile via API"""
    try:
        data = request.get_json()
        
        # Validate required fields
        required_fields = ['full_name', 'email', 'phone']
        BaseRouteHandler.validate_required_fields(data, required_fields)
...
@owner_profile_bp.route('/change_password', methods=['POST'])
@login_required
@owner_required
@handle_validation_errors
def change_password():
...
@owner_profile_bp.route('/upload-avatar', methods=['POST'])
@login_required
@owner_required
@handle_validation_errors
def upload_avatar():
    ...
    try:
        ...
        # Validate file
        BaseRouteHandler.validate_file_upload(file)
...
@owner_profile_bp.route('/remove-avatar', methods=['POST'])
@login_required
@owner_required
@handle_api_errors
def remove_avatar():
...
@owner_profile_bp.route('/check-username', methods=['POST'])
@login_required
@handle_api_errors
def check_username():
    ...
    try:
        ...
        return jsonify({"error": "Có lỗi xảy ra khi kiểm tra username"}), 500


@owner_profile_bp.route('/check-email', methods=['POST'])
@login_required
@handle_api_errors
def check_email():
...
# =============================================================================
# BOOKING SIMULATION (for owners viewing as renters)
# =============================================================================

@owner_profile_bp.route('/book-home/<int:home_id>', methods=['GET', 'POST'])
@owner_profile_bp.route('/book-home', methods=['GET', 'POST'])
@owner_required
@handle_web_errors
def book_home(home_id=None):
    """Book home (for owners viewing as renters)"""
    if request.method == 'POST':
        return handle_book_home_post(home_id)
    
    # GET request - show booking form
    if home_id:
        from app.models.models import Home
        home = Home.query.get_or_404(home_id)
        return render_template('owner/book_home.html', home=home)
    
    return render_template('owner/book_home.html')
...
def handle_profile_update():
    ...
    try:
        ...
        if not full_name:
            ...
            return redirect(url_for('owner_profile.profile'))
        ...
        from app.utils.notification_helpers import notify_profile_updated
...
def handle_book_home_post(home_id):
    """Handle booking form submission"""
    try:
        # This is a simulation for owners viewing as renters
        # In a real implementation, this would create actual bookings
        
        flash('Đây là chế độ xem thử nghiệm. Booking thực tế chỉ dành cho Renter.', 'info')
        return redirect(url_for('owner_dashboard.dashboard'))
        
    except Exception as e:
        current_app.logger.error(f"Error in book home simulation: {str(e)}")
...
def get_profile_stats():
    ...
    # Get owner's homes
    from app.models.models import Home, Booking
