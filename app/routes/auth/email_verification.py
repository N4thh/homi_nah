...
def renter_verify_otp():
    ...
    try:
        ...
        return jsonify({
            ...
            'redirect_url': url_for('renter_dashboard.dashboard')
        })
...
def verify_otp():
    ...
    try:
        ...
        redirect_url = url_for('owner_dashboard.dashboard') if user.is_owner() else url_for('renter_dashboard.dashboard')
        return jsonify({
            'success': True, 
            'message': 'Email đã được xác thực thành công!',
