...
# CSRF protection is handled automatically by Flask-WTF
import requests
import json
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            ...
            if isinstance(user, Admin):
                ...
                next_page = url_for('admin_dashboard.dashboard', login_success='1')
                
            elif isinstance(user, Owner):
                session['user_role'] = 'owner'
                # Kiểm tra xác thực email cho owner
                if not user.email_verified and user.first_login:
                    next_page = url_for('owner_profile.profile', login_success='1')
                else:
                    next_page = url_for('owner_profile.profile', login_success='1')
                
            else:  # Renter
                session['user_role'] = 'renter'
                # Kiểm tra xác thực email cho renter
                if not user.email_verified and user.first_login:
                    next_page = url_for('renter_dashboard.verify_email')
                else:
            ...
            from app.utils.notification_helpers import notify_login_success
...
@login_required
def logout():
    from app.utils.notification_helpers import notify_logout_success
    logout_user()
...
def callback_google():
    ...
    if existing_renter:
        ...
        if not existing_renter.email_verified:
...
def complete_google_signup():
    ...
    form_data = {
        ...
        'full_name': session.get('google_name', '')  # Pre-populate with Google name
    }
    ...
    if request.method == 'POST':
        ...
        if not all([full_name, phone]):
            ...
            form_data = {
                'full_name': full_name or session.get('google_name', ''),
                'phone': phone,
                'email': session.get('google_email', ''),
                'google_name': session.get('google_name', '')
            }
        ...
        new_renter = Renter(
            ...
            is_google=True,  # Mark as Google user
            email_verified=False,  # Bắt buộc xác thực email
            first_login=True
        )
        ...
        flash(f"Tài khoản của bạn đã được tạo thành công! Vui lòng xác thực email để tiếp tục.", "success")
        return redirect(url_for('renter_dashboard.verify_email'))
