...
# from payos.types import CreatePaymentLinkRequest, ItemData  # Comment out if not available
...
class PayOSService:
    ...
    def create_payment_link(self, order_code, amount: int, description: str, 
        ...
        try:
            ...
            # Tạo CreatePaymentLinkRequest object theo đúng SDK
            payment_data = CreatePaymentLinkRequest(
                order_code=order_code_int,  # Phải là số nguyên
                amount=int(amount),
            ...
            print(f"[DEBUG] CreatePaymentLinkRequest created: order_code={payment_data.order_code}, amount={payment_data.amount}")
    ...
    def get_payment_info(self, order_code: int) -> Dict[str, Any]:
        ...
        """
        try:
            result = self.payos.getPaymentLinkInformation(order_code)
            return {
                'success': True,
                'data': result
            }
    ...
    def cancel_payment(self, order_code: int, reason: str = "Hủy đơn hàng") -> Dict[str, Any]:
        ...
        """
        try:
            result = self.payos.cancelPaymentLink(order_code, reason)
            return {
                'success': True,
                'data': result
            }
