...
function initializeDateTimePlaceholders() {
  ...
  const bookingFormDateInputs = document.querySelectorAll(
...
function toggleCustomPlaceholder(input) {
  const placeholder = document.querySelector(
    `.custom-placeholder[data-for="${input.id}"]`
  );
  if (placeholder) {
    if (input.value) {
...
function initializeGuestSelectors() {
  ...
  document.addEventListener("click", function (event) {
    if (
      ...
      !event.target.matches(".guest-display")
    ) {
...
function updateGuestCount(formType, target, action) {
  const countElement = document.getElementById(
    `${formType}-${target}-count-${formType}`
  );
  const inputElement = document.getElementById(
    `${formType}-${target}-input-${formType}`
  );
  ...
  const decreaseButton = countElement
    .closest(".guest-controls")
...
function updateGuestDisplay(formType) {
  const adultsCount = parseInt(
    document.getElementById(`${formType}-adults-count-${formType}`).textContent
  );
  const childrenCount = parseInt(
    document.getElementById(`${formType}-children-count-${formType}`)
      .textContent
  );

  document.getElementById(
    `${formType}-guests-rooms-${formType}`
  ).value = `${adultsCount} người lớn, ${childrenCount} Trẻ em`;
}
...
function adjustNavbarTextColor() {
  ...
  console.log(
    `Navbar background: ${backgroundColor}, Luminance: ${luminance.toFixed(
      3
    )}, Theme: ${navbar.getAttribute("data-theme")}`
  );
}
  ...
  function renderMonth(calendarDays, calendarTitle, month, year) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (cellDate < today) {
        ...
        if (
          selectedCheckin &&
          cellDate.getTime() === selectedCheckin.getTime()
        ) {
          dayCell.classList.add("selected", "checkin");
        }
        if (
          selectedCheckout &&
          cellDate.getTime() === selectedCheckout.getTime()
        ) {
        ...
        if (
          selectedCheckin &&
          selectedCheckout &&
          cellDate > selectedCheckin &&
          cellDate < selectedCheckout
        ) {
...
function setupGlobalDailyDateTimeSelector() {
  ...
  const checkoutInput = document.getElementById(
    "global-checkout-display-daily"
  );
  const calendarDropdown = document.getElementById(
    "global-datetime-dropdown-daily"
  );
  ...
  function renderCalendar() {
    ...
    const calendarDays1 = document.getElementById(
      "global-calendar-days-daily-1"
    );
    const calendarDays2 = document.getElementById(
      "global-calendar-days-daily-2"
    );
    const calendarTitle1 = document.getElementById(
      "global-calendar-title-daily-1"
    );
    const calendarTitle2 = document.getElementById(
      "global-calendar-title-daily-2"
    );
...
function initializeSearchFormValidation() {
  ...
  if (hourlyForm) {
    hourlyForm.addEventListener("submit", function (e) {
      const checkinDate = document.getElementById(
        "global-checkin-date-hourly"
      ).value;
      const checkinTime = document.getElementById(
        "global-checkin-time-hourly"
      ).value;
      const hoursDuration = document.getElementById(
        "global-hours-duration-hourly"
      ).value;
  ...
  if (dailyForm) {
    dailyForm.addEventListener("submit", function (e) {
      const checkinDate = document.getElementById(
        "global-checkin-date-daily"
      ).value;
      const checkoutDate = document.getElementById(
        "global-checkout-date-daily"
      ).value;
...
function updateSearchPlaceholders() {
  ...
  const hourlyCheckinDisplay = document.getElementById(
    "global-datetime-hourly"
  );
  const hourlyCheckoutDisplay = document.getElementById(
    "global-checkout-display-hourly"
  );
  const hourlyCheckinDate = document.getElementById(
    "global-checkin-date-hourly"
  ).value;
  const hourlyCheckinTime = document.getElementById(
    "global-checkin-time-hourly"
  ).value;
  const hourlyDuration = document.getElementById(
    "global-hours-duration-hourly"
  ).value;
  ...
  if (hourlyCheckinDisplay) {
    if (hourlyCheckinDate && hourlyCheckinTime && hourlyDuration) {
      ...
      hourlyCheckoutDisplay.value = `${checkoutDayName} - ${checkoutDay} tháng ${checkoutMonth} - ${checkoutHour
        .toString()
        .padStart(2, "0")}:00`;
    } else {
  ...
  const dailyCheckoutDisplay = document.getElementById(
    "global-checkout-display-daily"
  );
  const dailyCheckinDate = document.getElementById(
    "global-checkin-date-daily"
  ).value;
  const dailyCheckoutDate = document.getElementById(
    "global-checkout-date-daily"
  ).value;
...
document.addEventListener("DOMContentLoaded", function () {
  if (
    document.getElementById("hourlyDateCompact") &&
