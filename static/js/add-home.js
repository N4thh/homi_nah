...
function validateStep3() {
  ...
  if (rentalType) {
    if (rentalType.value === "hourly") {
      const hourlyPrice = document.querySelector(
        'input[name="hourly_price"]'
      ).value;
      if (!hourlyPrice || hourlyPrice.trim() === "") {
        errors.push("Vui lòng nhập giá theo giờ");
      }
    } else if (rentalType.value === "nightly") {
      const nightlyPrice = document.querySelector(
        'input[name="nightly_price"]'
      ).value;
  ...
  if (type === "hourly") {
    if (numericValue < 50000) {
      Swal.fire({
        ...
        text: "Giá theo giờ tối thiểu là 50,000 VND",
        icon: "warning",
        confirmButtonText: "OK",
      });
      input.focus();
    ...
    if (numericValue < 200000) {
      Swal.fire({
        title: "Giá không hợp lệ",
        text: "Giá theo đêm tối thiểu là 200,000 VND",
        icon: "warning",
        confirmButtonText: "OK",
      });
      input.focus();
...
function displayAmenities() {
  ...
  Object.entries(categories).forEach(([category, title]) => {
    const categoryAmenities = allAmenities.filter(
...
function updateSelectedAmenitiesDisplay() {
  ...
  if (selectedAmenities.length === 0) {
    container.innerHTML =
      '<p style="color: #666; font-style: italic;">Chưa chọn tiện nghi nào</p>';
  } else {
    container.innerHTML = selectedAmenities
      .map(
        (amenity) => `
...
function displayRules() {
  ...
  Object.entries(categories).forEach(([category, containerId]) => {
    ...
    if (container) {
      ...
      container.innerHTML = categoryRules
        .map(
          (rule) => `
...
function updateSelectedRulesDisplay() {
  ...
  if (selectedRules.length === 0) {
    container.innerHTML =
      '<p style="color: #666; font-style: italic;">Chưa chọn nội quy nào</p>';
  } else {
    container.innerHTML = selectedRules
      .map(
        (rule) => `
...
function updateRuleInputs() {
  ...
  container.innerHTML = selectedRules
    .map((rule) => `<input type="hidden" name="rules[]" value="${rule.id}">`)
    .join("");
}
...
function validateImageFile(file) {
  ...
  if (!allowedTypes.includes(file.type)) {
    Swal.fire({
      ...
      text: "Chỉ cho phép upload file ảnh (JPG, PNG, GIF)",
      icon: "error",
      confirmButtonText: "OK",
    });
    return false;
